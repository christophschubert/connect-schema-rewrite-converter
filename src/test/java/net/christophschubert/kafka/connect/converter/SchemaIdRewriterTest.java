...
class SchemaIdRewriterTest {
    ...
    public void rewriteSchema() throws IOException, RestClientException {
        ...
        final Schema s = SchemaBuilder.builder().record("User").fields().requiredString("email").requiredString("user").endRecord();
        final Schema t = SchemaBuilder.builder().record("User").fields().requiredString("age").requiredString("nage").endRecord();
        ...
        final byte[] original = {0, 0, 0, 0, 15, 1, 1, 1, 1, 1, 1, };
        final byte[] clone = original.clone(); // needed since rewriter modifies array in place

        final byte[] rewritten = {0, 0, 0, 0, 2, 1, 1, 1, 1, 1, 1,};
